...
Membership payments are managed from the admin console at 会员与支付 → 支付配置. Add one or more payment providers (易支付/EasyPay, official Alipay, official WeChat Pay, Stripe, Airwallex, or NOWPayments) with their merchant credentials; the bot creates orders through the first enabled provider. Provider webhooks are exposed under `/api/payments/webhook/<provider>` and must be reachable over public HTTPS, so set each provider's callback base URL (or `PUBLIC_BASE_URL`) to the deployed domain. The legacy environment variables `NOWPAYMENTS_API_KEY`, `NOWPAYMENTS_IPN_SECRET`, and `PUBLIC_BASE_URL` still work as a fallback when no provider is configured in the console; the NOWPayments IPN endpoint remains `POST /api/payments/nowpayments/ipn`.
